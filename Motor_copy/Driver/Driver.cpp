...
Driver::Driver(PinName left_motor,PinName right_motor,PinName left_encoder,PinName right_encoder):m_left(left_motor, p6, p5),
    ...
    controller_left(0.1, 0.1, 0.3, RATE),
    controller_right(0.1, 0.1, 0.3, RATE),
    controller_diff(0.3, 0.1, 0.4, RATE)
{
    controller_left.setInputLimits(0.0, 50);
    controller_left.setOutputLimits(0.0, 1.0);
    controller_left.setBias(0.0);
    controller_left.setMode(AUTO_MODE);

    controller_right.setInputLimits(0, 50);
    controller_right.setOutputLimits(0.0, 1.0);
    controller_right.setBias(0.0);
    controller_right.setMode(AUTO_MODE);


    controller_diff.setInputLimits(-500, 500);
    controller_diff.setOutputLimits(-1, 1);
...
void Driver::forward(float speed)
{
    ...
    m_right.speed(controller_right.compute() - controller_diff.compute());
    m_left.speed(controller_left.compute() + controller_diff.compute());
...
void Driver::turn(int direction, float degree)
{
    ...
    int pulseCount = degree / 90 * 36;   // 90 degree is 36 pulses;
